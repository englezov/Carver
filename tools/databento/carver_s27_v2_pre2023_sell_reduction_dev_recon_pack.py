...
PROCESS_DOC = ROOT / "docs/process/CARVER_S27_ZN_V2_PRE2023_SELL_REDUCTION_DEV_RECON_PACK_BUILD_RESULT_2026-06-11.md"

AUTHORIZATION = "S27_V2_CONSOLIDATED_LOCAL_ONLY_PRE2023_SELL_REDUCTION_DEVELOPMENT_RECON_IMPLEMENTATION_AND_RUN_GATE"
STATUS = "LOCAL_PRE2023_SELL_REDUCTION_DEV_RECON_PACK_DECLARED_NOT_RESULT"
...
DECISION_TIMESTAMPS = (
    ...
    "2022-01-06T02:00:00Z",
    "2022-01-06T09:00:00Z",
    "2022-01-07T02:00:00Z",
    "2022-01-07T04:00:00Z",
    "2022-01-10T02:00:00Z",
    "2022-01-10T04:00:00Z",
    ...
    "2022-01-14T06:00:00Z",
    "2022-01-17T03:00:00Z",
    "2022-01-17T05:00:00Z",
    "2022-01-18T01:00:00Z",
    "2022-01-18T05:00:00Z",
    "2022-01-20T04:00:00Z",
...
FILL_TIMESTAMPS = (
    ...
    "2022-01-05T15:00:00Z",
    "2022-01-06T01:00:00Z",
    "2022-01-06T03:00:00Z",
    "2022-01-06T10:00:00Z",
    "2022-01-07T03:00:00Z",
    "2022-01-07T05:00:00Z",
    "2022-01-10T03:00:00Z",
    "2022-01-10T05:00:00Z",
    "2022-01-10T07:00:00Z",
    ...
    "2022-01-14T07:00:00Z",
    "2022-01-17T04:00:00Z",
    "2022-01-17T06:00:00Z",
    "2022-01-18T02:00:00Z",
    "2022-01-18T06:00:00Z",
    "2022-01-20T05:00:00Z",
    "2022-01-24T02:00:00Z",
)
VALUATION_TIMESTAMPS = (
    ...
    "2022-01-06T02:00:00Z",
    "2022-01-06T04:00:00Z",
    "2022-01-06T11:00:00Z",
    "2022-01-07T04:00:00Z",
    "2022-01-07T06:00:00Z",
    "2022-01-10T04:00:00Z",
    "2022-01-10T06:00:00Z",
    ...
    "2022-01-17T05:00:00Z",
    "2022-01-17T07:00:00Z",
    "2022-01-18T03:00:00Z",
    "2022-01-18T07:00:00Z",
    "2022-01-20T06:00:00Z",
    "2022-01-24T03:00:00Z",
)
COPY_FAMILIES = (
...
def main() -> None:
    ...
    _write_json(OUTPUT_PACK / "S27_V2_PRE2023_SELL_REDUCTION_DEV_RECON_DECLARED_INPUT_PACK_MANIFEST.json", manifest)
    _write_sha256s(OUTPUT_PACK / "S27_V2_PRE2023_SELL_REDUCTION_DEV_RECON_DECLARED_INPUT_PACK_SHA256SUMS.txt", OUTPUT_PACK)
...
def _hourly_pack_row(row: dict[str, str], role: str, index: int) -> dict[str, Any]:
    return {
        ...
        "row_locator": f"S27V2_PRE2023_SELL_REDUCTION_DEV_RECON_ZN_HOURLY_{role}_{index:04d}_{row['derived_completed_bar_end_utc'].replace('-', '').replace(':', '')}_{row['raw_symbol']}",
...
def _session_bounds(completed_timestamp: str) -> tuple[str, str]:
    completed = datetime.strptime(completed_timestamp, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
    if completed.hour >= 22:
        session_start = completed.replace(hour=22, minute=0, second=0, microsecond=0)
        session_end = session_start + timedelta(hours=23)
    else:
        session_end = completed.replace(hour=21, minute=0, second=0, microsecond=0)
        session_start = session_end - timedelta(hours=23)
    return _format_utc(session_start), _format_utc(session_end)
...
def _manifest(hourly: dict[str, dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        "artifact": "S27_V2_PRE2023_SELL_REDUCTION_DEV_RECON_DECLARED_INPUT_PACK_MANIFEST",
        "authorization": AUTHORIZATION,
        "status": STATUS,
        "lane": "SOURCE_NATIVE_FUTURES",
        "selected_slice_rule": "MINIMUM_OLDEST_2022_SELL_REDUCTION_DEV_RECON_FIRST_REDUCTION_SEGMENT_AFTER_WARMUPS_PRESERVE_2023_FOR_TEST",
        ...
        "history_evidence": {
            ...
            "sell_reduction_stop_reason": "FIRST_CARRIED_POSITION_REDUCTION_AT_2022-01-24T01:00:00Z_WITH_CURRENT_34_DESIRED_33",
        },
...
def _process_doc(manifest: dict[str, Any]) -> str:
    return f"""# S27_V2 Pre-2023 Sell Reduction Development/Reconciliation Pack Build Result
...
```text
{manifest['history_evidence']['sell_reduction_stop_reason']}
```
