...
RUN_ID = "20260613_S27_V2_2023_TEST_STANDING_MARKET_ORDER_TBBO_FAILED_WINDOW_RETRY"
AUTHORIZATION = "S27_V2_STANDING_BOUNDED_MARKET_ORDER_TBBO_SPREAD_EVIDENCE_POLICY_RETRY"
LOOKBACK_SECONDS = 60
LOOKAHEAD_SECONDS = 5
...
STANDING_BATCH_STATUS = STANDING_BATCH_ROOT / "status/20260613_S27_V2_2023_TEST_STANDING_MARKET_ORDER_TBBO_BATCH_status.json"
OUTPUT_ROOT = (
    ROOT
    / "docs/researchops/s27_v2_market_spread_evidence/ZN/"
    / "20260613_2023_test_standing_market_order_tbbo_failed_window_retry"
)
PROCESS_RECORD = (
    ROOT
    / "docs/process/CARVER_S27_ZN_V2_2023_TEST_STANDING_MARKET_ORDER_TBBO_FAILED_WINDOW_RETRY_2026-06-13.md"
)


@dataclass(frozen=True)
class StandingFailedWindowRetryConfig:
...
def main() -> None:
    status = run_standing_failed_window_retry(StandingFailedWindowRetryConfig(execution_authorized=True))
    print(status["status"])
...
def run_standing_failed_window_retry(config: StandingFailedWindowRetryConfig) -> dict[str, Any]:
    ...
    for requirement in requirements:
        ...
        try:
            if raw_dbn.exists() or raw_csv.exists():
                raise CarverBlocked(f"Standing retry raw outputs already exist for row {row_index}; refusing duplicate provider request")
    ...
    if status["status"] != "PASS_STANDING_BOUNDED_DATABENTO_TBBO_FAILED_WINDOW_RETRY_SELECTED_NOT_RESULT":
        raise CarverBlocked("S27 v2 TEST standing TBBO failed-window retry failed closed; see retry status ledger")
    return status


def _validate_config(config: StandingFailedWindowRetryConfig) -> None:
    if not config.execution_authorized:
        raise CarverBlocked("S27 v2 TEST standing TBBO retry is not authorized")
    if config.requirements_ledger.resolve() != REQUIREMENTS_LEDGER.resolve():
        raise CarverBlocked("S27 v2 TEST standing TBBO retry must use the locked requirements ledger")
    if config.output_root.resolve() != OUTPUT_ROOT.resolve():
        raise CarverBlocked("S27 v2 TEST standing TBBO retry output root is locked")


def _failed_requirements(path: Path) -> list[dict[str, str]]:
    status = _read_json(STANDING_BATCH_STATUS)
    current_request_indices = {str(index) for index in status.get("current_request_row_indices", ())}
    failed_indices = tuple(
        str(index)
        for index in status.get("failed_row_indices", ())
        if str(index) in current_request_indices
    )
    if not failed_indices:
        raise CarverBlocked("S27 v2 TEST standing TBBO retry has no failed rows to retry")
    ...
    for row_index in failed_indices:
        ...
        if row["tbbo_requirement_status"] != "REQUIRES_BOUNDED_DATABENTO_TBBO_EVIDENCE":
            raise CarverBlocked(f"S27 v2 TEST standing retry row {row_index} is not a missing-TBBO requirement")
        if not _is_zn_quarterly_raw_symbol(row["raw_symbol"]):
            raise CarverBlocked("S27 v2 TEST standing retry raw-symbol drift")
        if row["provider"] != PROVIDER or row["dataset"] != DATASET or row["schema"] != SCHEMA:
            raise CarverBlocked("S27 v2 TEST standing retry provider/schema drift")
        if not row["decision_timestamp_utc"].startswith("2023-") or not row["fill_candidate_timestamp_utc"].startswith("2023-"):
            raise CarverBlocked("S27 v2 TEST standing retry rows must stay in 2023")
        if row["source_faithful_evidence_claimed"] != "FALSE" or row["result_interpretation_authorized"] != "FALSE":
            raise CarverBlocked("S27 v2 TEST standing retry requirements must not claim result/source-faithful authority")
        ...
        if row["row_hash"] != canonical_sha256({key: value for key, value in row.items() if key != "row_hash"}):
            raise CarverBlocked("S27 v2 TEST standing retry requirement row hash drift")
...
def _validate_original_request_window(row: dict[str, str]) -> None:
    ...
    if row["request_start_utc"] != expected_start or row["request_end_utc"] != expected_end:
        raise CarverBlocked("S27 v2 TEST standing retry source request window must equal fill timestamp +/- 5 seconds")
...
def _request_manifest(requirements: list[dict[str, str]], registry_rows: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "schema": SCHEMA,
        "stype_in": STYPE_IN,
        "current_retry_row_indices": [row["row_index"] for row in requirements],
        ...
        "standing_batch_status": _rel(STANDING_BATCH_STATUS),
        "standing_batch_status_sha256": _sha256(STANDING_BATCH_STATUS),
        "current_retry_request_count": len(requirements),
...
def _status_payload(
    ...
    return {
        "authorization": AUTHORIZATION,
        "status": (
            "PASS_STANDING_BOUNDED_DATABENTO_TBBO_FAILED_WINDOW_RETRY_SELECTED_NOT_RESULT"
            if not failed
            else "FAIL_CLOSED_STANDING_BOUNDED_DATABENTO_TBBO_FAILED_WINDOW_RETRY_INCOMPLETE_NOT_RESULT"
        ),
        "current_retry_request_count": len(requirements),
        "current_retry_row_indices": [row["row_index"] for row in requirements],
        ...
        "provider_api_access": "STANDING_POLICY_BOUNDED_DATABENTO_TBBO_RETRY_ONLY",
...
def _provenance_text(status: dict[str, Any]) -> str:
    return f"""# S27 V2 2023 TEST Standing Market-Order TBBO Failed-Window Retry Provenance
...
Current retry rows: {status['current_retry_request_count']}
...
This retry was limited to failed rows from the standing batch status. Selection remained at-or-before fill; post-fill quote selection stayed disallowed.
"""


def _process_record(status: dict[str, Any]) -> str:
    return f"""# S27 V2 ZN 2023 TEST Standing Market-Order TBBO Failed-Window Retry
...
Current retry count: `{status['current_retry_request_count']}`
...
def _read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise CarverBlocked("S27 v2 TEST standing retry JSON payload must be an object")
    return payload
