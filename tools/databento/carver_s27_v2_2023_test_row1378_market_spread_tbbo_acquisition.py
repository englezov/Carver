...
PRIMARY_RUN_ID = "20260621_S27_V2_2023_TEST_ROW1378_MARKET_ORDER_TBBO"
RETRY_RUN_ID = "20260621_S27_V2_2023_TEST_ROW1378_MARKET_ORDER_TBBO_FAILED_WINDOW_RETRY"
AUTHORIZATION = "S27_V2_CONTROLLED_LOCAL_ONLY_2023_TEST_CONTINUATION_FAST_OPERATIONAL_RUNNER_GATE"
ROW_INDEX = "1378"
RAW_SYMBOL = "ZNM3"
PRIMARY_OUTPUT_ROOT = (
    ROOT
    / "docs/researchops/s27_v2_market_spread_evidence/ZN/"
    / "20260621_2023_test_row1378_market_order_tbbo"
)
RETRY_OUTPUT_ROOT = (
    ROOT
    / "docs/researchops/s27_v2_market_spread_evidence/ZN/"
...
PRIMARY_PROCESS_RECORD = (
    ROOT / "docs/process/CARVER_S27_ZN_V2_2023_TEST_ROW1378_MARKET_ORDER_TBBO_EVIDENCE_2026-06-21.md"
)
RETRY_PROCESS_RECORD = (
    ROOT
    / "docs/process/CARVER_S27_ZN_V2_2023_TEST_ROW1378_MARKET_ORDER_TBBO_FAILED_WINDOW_RETRY_EVIDENCE_2026-06-21.md"
)
...
class Row1378TBBOConfig:
    execution_authorized: bool
    run_id: str = PRIMARY_RUN_ID
    fail_closed_ledger: Path = RUN_FAIL_CLOSED_LEDGER
    output_root: Path = PRIMARY_OUTPUT_ROOT
    process_record: Path = PRIMARY_PROCESS_RECORD
    lookback_seconds: int = 5
    retry_mode: bool = False


def main() -> None:
    retry = "--retry-60s" in sys.argv
    status = run_row1378_tbbo_acquisition(
        Row1378TBBOConfig(
            execution_authorized=True,
            run_id=RETRY_RUN_ID if retry else PRIMARY_RUN_ID,
            output_root=RETRY_OUTPUT_ROOT if retry else PRIMARY_OUTPUT_ROOT,
            process_record=RETRY_PROCESS_RECORD if retry else PRIMARY_PROCESS_RECORD,
            lookback_seconds=60 if retry else 5,
            retry_mode=retry,
        )
    )
    print(status["status"])
    print(f"output_root={Path(status['output_root']).relative_to(ROOT).as_posix()}")
    print(f"selected_rows={status['selected_row_count']}")
    print(f"failed_rows={status['failed_row_count']}")


def run_row1378_tbbo_acquisition(config: Row1378TBBOConfig) -> dict[str, Any]:
    _validate_config(config)
    _ensure_output_dirs(config.output_root)
    requirement = _row1378_requirement(config.fail_closed_ledger, config.lookback_seconds)
    active_fail_closed_snapshot = _write_active_fail_closed_snapshot(config.fail_closed_ledger, config.output_root)
    client = db.Historical(_read_databento_key())

    _write_json(
        config.output_root / "manifest" / f"{config.run_id}_request_manifest.json",
        _request_manifest(requirement, active_fail_closed_snapshot, config),
    )

    raw_dbn = config.output_root / "raw_provider_output" / f"{config.run_id}_row_{ROW_INDEX}.dbn"
    raw_csv = config.output_root / "raw_provider_output" / f"{config.run_id}_row_{ROW_INDEX}_tbbo_dataframe.csv"
    ...
    try:
        if raw_dbn.exists() or raw_csv.exists():
            raise CarverBlocked("Row 1378 TBBO raw outputs already exist; refusing duplicate provider request")
        ...
        if not raw_dbn.exists() or raw_dbn.stat().st_size <= 0:
            raise RuntimeError("Databento returned no row 1378 raw TBBO DBN bytes")
        df = store.to_df()
        df.to_csv(raw_csv)
    ...
    selected_path = config.output_root / "ledger" / f"{config.run_id}_selected_spread_registry.csv"
    provider_path = config.output_root / "provider_condition" / f"{config.run_id}_provider_condition_ledger.csv"
    raw_path = config.output_root / "raw_provider_output" / f"{config.run_id}_raw_output_registry.csv"
    status_path = config.output_root / "status" / f"{config.run_id}_status.json"
    provenance_path = config.output_root / "provenance" / f"{config.run_id}_provenance.md"
    sha_path = config.output_root / "hashes" / f"{config.run_id}_sha256.csv"
    ...
    status = _status_payload(selected_rows, provider_condition_rows, config)
    _write_json(status_path, status)
    provenance_path.write_text(_provenance_text(status), encoding="ascii")
    config.process_record.write_text(_process_record(status, config), encoding="ascii")
    _write_sha256_manifest(sha_path, config.output_root)

    if status["status"] != "PASS_ROW1378_BOUNDED_DATABENTO_TBBO_SELECTED_NOT_RESULT":
        raise CarverBlocked("S27 v2 TEST row 1378 TBBO evidence failed closed; see status ledger")
    return status


def _validate_config(config: Row1378TBBOConfig) -> None:
    if not config.execution_authorized:
        raise CarverBlocked("S27 v2 TEST row 1378 TBBO acquisition is not authorized")
    if config.fail_closed_ledger.resolve() != RUN_FAIL_CLOSED_LEDGER.resolve():
        raise CarverBlocked("S27 v2 TEST row 1378 TBBO acquisition must use the locked fail-closed ledger")
    allowed = {
        (PRIMARY_RUN_ID, PRIMARY_OUTPUT_ROOT.resolve(), PRIMARY_PROCESS_RECORD.resolve(), 5, False),
        (RETRY_RUN_ID, RETRY_OUTPUT_ROOT.resolve(), RETRY_PROCESS_RECORD.resolve(), 60, True),
    }
    observed = (
        config.run_id,
        config.output_root.resolve(),
        config.process_record.resolve(),
        config.lookback_seconds,
        config.retry_mode,
    )
    if observed not in allowed:
        raise CarverBlocked("S27 v2 TEST row 1378 TBBO acquisition output/mode is locked")


def _row1378_requirement(path: Path, lookback_seconds: int) -> dict[str, str]:
    rows = [row for row in _read_csv(path) if row["row_index"] == ROW_INDEX]
    if len(rows) != 1:
        raise CarverBlocked("S27 v2 TEST row 1378 fail-closed ledger must contain exactly one active blocker row")
    fail_row = rows[0]
    expected = {
        ...
        "order_side": "SELL",
        "fill_candidate_timestamp_utc": "2023-03-31T00:00:00Z",
    ...
    for key, value in expected.items():
        if fail_row.get(key) != value:
            raise CarverBlocked(f"S27 v2 TEST row 1378 active blocker drift: {key}")
    if fail_row["row_hash"] != canonical_sha256(_row1378_fail_closed_hash_payload(fail_row)):
        raise CarverBlocked("S27 v2 TEST row 1378 active blocker row hash drift")
    ...
    requirement = {
        ...
        "schema": SCHEMA,
        "stype_in": STYPE_IN,
        "request_start_utc": _format_z(fill_time - timedelta(seconds=lookback_seconds)),
...
def _row1378_fail_closed_hash_payload(row: dict[str, str]) -> dict[str, Any]:
    return {
...
def _request_manifest(
    ...
    active_fail_closed_snapshot: Path,
    config: Row1378TBBOConfig,
) -> dict[str, Any]:
    return {
        "authorization": AUTHORIZATION,
        "standing_authorization": "S27_V2_STANDING_BOUNDED_MARKET_ORDER_TBBO_SPREAD_EVIDENCE_POLICY",
        "run_id": config.run_id,
        "retry_mode": "TRUE" if config.retry_mode else "FALSE",
        ...
        "active_fail_closed_ledger_snapshot": _rel(active_fail_closed_snapshot),
        "active_fail_closed_ledger_snapshot_sha256": _sha256(active_fail_closed_snapshot),
        "row_index": ROW_INDEX,
        "raw_symbol": RAW_SYMBOL,
        ...
        "scope": "ROW1378_ONLY_BOUNDED_2023_TEST_MARKET_ORDER_TBBO_REQUIREMENT",
...
def _write_active_fail_closed_snapshot(path: Path, output_root: Path) -> Path:
    rows = [row for row in _read_csv(path) if row["row_index"] == ROW_INDEX]
    if len(rows) != 1:
        raise CarverBlocked("S27 v2 TEST row 1378 active blocker snapshot requires exactly one active row")
    snapshot_prefix = RETRY_RUN_ID if output_root.resolve() == RETRY_OUTPUT_ROOT.resolve() else PRIMARY_RUN_ID
    snapshot_path = output_root / "manifest" / f"{snapshot_prefix}_active_fail_closed_ledger_snapshot.csv"
    source_text = path.read_text(encoding="ascii").splitlines()
    if not source_text:
        raise CarverBlocked("S27 v2 TEST row 1378 active blocker snapshot source is empty")
    header = source_text[0]
    snapshot_bytes = (header + "\n" + ",".join(rows[0][column] for column in header.split(",")) + "\n").encode(
        "ascii"
    )
    snapshot_path.write_bytes(snapshot_bytes)
    if _sha256(snapshot_path) != _sha256(path):
        raise CarverBlocked("S27 v2 TEST row 1378 active blocker snapshot hash drift")
...
def _status_payload(
    ...
    return {
        ...
        "lookback_seconds": config.lookback_seconds,
        "status": (
            "PASS_ROW1378_BOUNDED_DATABENTO_TBBO_SELECTED_NOT_RESULT"
            if not failed
            else "FAIL_CLOSED_ROW1378_BOUNDED_DATABENTO_TBBO_INCOMPLETE_NOT_RESULT"
        ),
        ...
        "provider_api_access": "ROW1378_ONLY_BOUNDED_DATABENTO_TBBO",
        "downloads": "ROW1378_ONLY_BOUNDED_TBBO_WINDOW",
        "broader_test_continuation": "NO",
...
def _provenance_text(status: dict[str, Any]) -> str:
    return f"""# S27 V2 2023 TEST Row 1378 Market-Order TBBO Evidence Provenance
...
Provider access was limited to the row-1378 ZNM3 TBBO quote window from the active fail-closed TEST artifact. This is not broad provider access, not a general historical download, not result evidence, and not source-faithful evidence.
"""


def _process_record(status: dict[str, Any], config: Row1378TBBOConfig) -> str:
    return f"""# S27 V2 ZN 2023 TEST Row 1378 Market-Order TBBO Evidence
...
This record is written under `S27_V2_CONTROLLED_LOCAL_ONLY_2023_TEST_CONTINUATION_FAST_OPERATIONAL_RUNNER_GATE` and the standing bounded market-order TBBO spread evidence policy.

Run id: `{config.run_id}`
Retry mode: `{status['retry_mode']}`
...
This evidence gate does not authorize broader provider/API access, general downloads, broader TEST continuation beyond the current declared pack, result interpretation, PnL evaluation, tuning, source-faithful evidence claims, VALIDATION, OOS, Lockbox, Forward, adapter work, deployment, trading, promotion, Git actions, or external audit packet preparation.
"""
