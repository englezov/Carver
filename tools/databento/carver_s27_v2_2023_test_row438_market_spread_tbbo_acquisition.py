...
RUN_ID = "20260613_S27_V2_2023_TEST_ROW438_MARKET_ORDER_TBBO"
AUTHORIZATION = "S27_V2_2023_TEST_ROW438_BOUNDED_MARKET_ORDER_TBBO_EVIDENCE_GATE"
ROW_INDEX = "438"
OUTPUT_ROOT = (
    ROOT
...
PROCESS_RECORD = (
    ROOT
    / "docs/process/CARVER_S27_ZN_V2_2023_TEST_ROW438_MARKET_ORDER_TBBO_EVIDENCE_2026-06-13.md"
)


@dataclass(frozen=True)
class Row438TBBOConfig:
...
def main() -> None:
    status = run_row438_tbbo_acquisition(Row438TBBOConfig(execution_authorized=True))
    print(status["status"])
...
def run_row438_tbbo_acquisition(config: Row438TBBOConfig) -> dict[str, Any]:
    _validate_config(config)
    _ensure_output_dirs(config.output_root)
    requirement = _row438_requirement(config.requirements_ledger)
    ...
    try:
        if raw_dbn.exists() or raw_csv.exists():
            raise CarverBlocked("Row 438 TBBO raw outputs already exist; refusing duplicate provider request")
        ...
        if not raw_dbn.exists() or raw_dbn.stat().st_size <= 0:
            raise RuntimeError("Databento returned no row 438 raw TBBO DBN bytes")
        df = store.to_df()
        df.to_csv(raw_csv)
    ...
    if status["status"] != "PASS_ROW438_BOUNDED_DATABENTO_TBBO_SELECTED_NOT_RESULT":
        raise CarverBlocked("S27 v2 TEST row 438 TBBO evidence failed closed; see status ledger")
    return status


def _validate_config(config: Row438TBBOConfig) -> None:
    if not config.execution_authorized:
        raise CarverBlocked("S27 v2 TEST row 438 TBBO acquisition is not authorized")
    if config.requirements_ledger.resolve() != REQUIREMENTS_LEDGER.resolve():
        raise CarverBlocked("S27 v2 TEST row 438 TBBO acquisition must use the locked requirements ledger")
    if config.output_root.resolve() != OUTPUT_ROOT.resolve():
        raise CarverBlocked("S27 v2 TEST row 438 TBBO acquisition output root is locked")


def _row438_requirement(path: Path) -> dict[str, str]:
    rows = [row for row in _read_csv(path) if row["row_index"] == ROW_INDEX]
    if len(rows) != 1:
        raise CarverBlocked("S27 v2 TEST row 438 TBBO requirement must contain exactly one row")
    row = rows[0]
    expected = {
        "decision_timestamp_utc": "2023-01-31T01:00:00Z",
        "fill_candidate_timestamp_utc": "2023-01-31T02:00:00Z",
        ...
        "schema": SCHEMA,
        "stype_in": STYPE_IN,
        "request_start_utc": "2023-01-31T01:59:55Z",
        "request_end_utc": "2023-01-31T02:00:05Z",
        "result_interpretation_authorized": "FALSE",
        "source_faithful_evidence_claimed": "FALSE",
    }
    for key, value in expected.items():
        if row.get(key) != value:
            raise CarverBlocked(f"S27 v2 TEST row 438 TBBO requirement drift: {key}")
    _validate_request_window(row)
    if row["row_hash"] != canonical_sha256({key: value for key, value in row.items() if key != "row_hash"}):
        raise CarverBlocked("S27 v2 TEST row 438 TBBO requirement row hash drift")
    return row
...
def _validate_request_window(row: dict[str, str]) -> None:
    ...
    if row["request_start_utc"] != expected_start or row["request_end_utc"] != expected_end:
        raise CarverBlocked("S27 v2 TEST row 438 TBBO request window must equal fill timestamp +/- 5 seconds")
...
def _request_manifest(requirement: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        "scope": "ROW438_ONLY_BOUNDED_2023_TEST_MARKET_ORDER_TBBO_REQUIREMENT",
...
def _status_payload(
    ...
    return {
        ...
        "standing_authorization": "S27_V2_STANDING_BOUNDED_MARKET_ORDER_TBBO_SPREAD_EVIDENCE_POLICY",
        "status": (
            "PASS_ROW438_BOUNDED_DATABENTO_TBBO_SELECTED_NOT_RESULT"
            if not failed
            else "FAIL_CLOSED_ROW438_BOUNDED_DATABENTO_TBBO_INCOMPLETE_NOT_RESULT"
        ),
        ...
        "provider_api_access": "ROW438_ONLY_BOUNDED_DATABENTO_TBBO",
        "downloads": "ROW438_ONLY_BOUNDED_TBBO_WINDOW",
...
def _provenance_text(status: dict[str, Any]) -> str:
    return f"""# S27 V2 2023 TEST Row 438 Market-Order TBBO Evidence Provenance
...
Provider access was limited to the row-438 ZNH3 TBBO quote window from the active requirements ledger. This is not broad provider access, not a general historical download, not result evidence, and not source-faithful evidence.
"""


def _process_record(status: dict[str, Any]) -> str:
    return f"""# S27 V2 ZN 2023 TEST Row 438 Market-Order TBBO Evidence
...
This record is written under `S27_V2_2023_TEST_ROW438_BOUNDED_MARKET_ORDER_TBBO_EVIDENCE_GATE` and the standing bounded market-order TBBO spread evidence policy.
