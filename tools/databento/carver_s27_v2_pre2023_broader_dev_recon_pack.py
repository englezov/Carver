...
PROCESS_DOC = ROOT / "docs/process/CARVER_S27_ZN_V2_PRE2023_BROADER_DEV_RECON_PACK_BUILD_RESULT_2026-06-11.md"

AUTHORIZATION = "S27_V2_CONSOLIDATED_LOCAL_ONLY_PRE2023_BROADER_DEVELOPMENT_RECON_IMPLEMENTATION_AND_RUN_GATE"
STATUS = "LOCAL_PRE2023_BROADER_DEV_RECON_PACK_DECLARED_NOT_RESULT"
...
def main() -> None:
    ...
    _write_json(OUTPUT_PACK / "S27_V2_PRE2023_BROADER_DEV_RECON_DECLARED_INPUT_PACK_MANIFEST.json", manifest)
    _write_sha256s(OUTPUT_PACK / "S27_V2_PRE2023_BROADER_DEV_RECON_DECLARED_INPUT_PACK_SHA256SUMS.txt", OUTPUT_PACK)
...
def _hourly_pack_row(row: dict[str, str], role: str, index: int) -> dict[str, Any]:
    return {
        ...
        "row_locator": f"S27V2_PRE2023_BROADER_DEV_RECON_ZN_HOURLY_{role}_{index:04d}_{row['derived_completed_bar_end_utc'].replace('-', '').replace(':', '')}_{row['raw_symbol']}",
...
def _session_row(completed_timestamp: str) -> dict[str, str]:
    ...
    return {
        ...
        "session_start_utc": "2022-01-02T22:00:00Z",
        "session_end_utc": "2022-01-03T21:00:00Z",
...
def _session_id(completed_timestamp: str) -> str:
    return "UTC_ZN_PRE2023_2022-01-02T22:00:00Z_2022-01-03T21:00:00Z"
...
def _manifest(hourly: dict[str, dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        "artifact": "S27_V2_PRE2023_BROADER_DEV_RECON_DECLARED_INPUT_PACK_MANIFEST",
        "authorization": AUTHORIZATION,
        "status": STATUS,
        "lane": "SOURCE_NATIVE_FUTURES",
        "selected_slice_rule": "MINIMUM_OLDEST_2022_BROADER_DEV_RECON_FIRST_SESSION_SEGMENT_AFTER_WARMUPS_PRESERVE_2023_FOR_TEST",
...
def _process_doc(manifest: dict[str, Any]) -> str:
    return f"""# S27_V2 Pre-2023 Broader Development/Reconciliation Pack Build Result
...
This pack is local-only and pre-2023. It selects the minimum oldest 2022 broader
Development/Reconciliation first-session segment after warmups/evidence are
populated: four decision rows, four one-hour fill candidates, and four
