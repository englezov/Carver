...
RUN_ID = "20260620_S27_V2_2023_TEST_ROW1374_MARKET_ORDER_TBBO"
AUTHORIZATION = "S27_V2_2023_TEST_ROW1374_BOUNDED_MARKET_ORDER_TBBO_EVIDENCE_AND_CONTINUATION_GATE"
ROW_INDEX = "1374"
RAW_SYMBOL = "ZNM3"
OUTPUT_ROOT = (
    ROOT
    / "docs/researchops/s27_v2_market_spread_evidence/ZN/"
    / "20260620_2023_test_row1374_market_order_tbbo"
)
PROCESS_RECORD = (
    ROOT / "docs/process/CARVER_S27_ZN_V2_2023_TEST_ROW1374_MARKET_ORDER_TBBO_EVIDENCE_2026-06-20.md"
)
...
def main() -> None:
    status = run_row1374_tbbo_acquisition(Row1374TBBOConfig(execution_authorized=True))
    print(status["status"])
...
def run_row1374_tbbo_acquisition(config: Row1374TBBOConfig) -> dict[str, Any]:
    _validate_config(config)
    _ensure_output_dirs(config.output_root)
    requirement = _row1374_requirement(config.fail_closed_ledger)
    ...
    try:
        if raw_dbn.exists() or raw_csv.exists():
            raise CarverBlocked("Row 1374 TBBO raw outputs already exist; refusing duplicate provider request")
        ...
        if not raw_dbn.exists() or raw_dbn.stat().st_size <= 0:
            raise RuntimeError("Databento returned no row 1374 raw TBBO DBN bytes")
        df = store.to_df()
        df.to_csv(raw_csv)
    ...
    if status["status"] != "PASS_ROW1374_BOUNDED_DATABENTO_TBBO_SELECTED_NOT_RESULT":
        raise CarverBlocked("S27 v2 TEST row 1374 TBBO evidence failed closed; see status ledger")
    return status


def _validate_config(config: Row1374TBBOConfig) -> None:
    if not config.execution_authorized:
        raise CarverBlocked("S27 v2 TEST row 1374 TBBO acquisition is not authorized")
    if config.fail_closed_ledger.resolve() != RUN_FAIL_CLOSED_LEDGER.resolve():
        raise CarverBlocked("S27 v2 TEST row 1374 TBBO acquisition must use the locked fail-closed ledger")
    if config.output_root.resolve() != OUTPUT_ROOT.resolve():
        raise CarverBlocked("S27 v2 TEST row 1374 TBBO acquisition output root is locked")


def _row1374_requirement(path: Path) -> dict[str, str]:
    rows = [row for row in _read_csv(path) if row["row_index"] == ROW_INDEX]
    if len(rows) != 1:
        raise CarverBlocked("S27 v2 TEST row 1374 fail-closed ledger must contain exactly one active blocker row")
    fail_row = rows[0]
    expected = {
    ...
    for key, value in expected.items():
        if fail_row.get(key) != value:
            raise CarverBlocked(f"S27 v2 TEST row 1374 active blocker drift: {key}")
    if fail_row["row_hash"] != canonical_sha256(_row1374_fail_closed_hash_payload(fail_row)):
        raise CarverBlocked("S27 v2 TEST row 1374 active blocker row hash drift")
...
def _row1374_fail_closed_hash_payload(row: dict[str, str]) -> dict[str, Any]:
    return {
...
def _request_manifest(requirement: dict[str, str], active_fail_closed_snapshot: Path) -> dict[str, Any]:
    return {
        ...
        "scope": "ROW1374_ONLY_BOUNDED_2023_TEST_MARKET_ORDER_TBBO_REQUIREMENT",
...
def _write_active_fail_closed_snapshot(path: Path, output_root: Path) -> Path:
    rows = [row for row in _read_csv(path) if row["row_index"] == ROW_INDEX]
    if len(rows) != 1:
        raise CarverBlocked("S27 v2 TEST row 1374 active blocker snapshot requires exactly one active row")
    ...
    if not source_text:
        raise CarverBlocked("S27 v2 TEST row 1374 active blocker snapshot source is empty")
    ...
    if _sha256(snapshot_path) != _sha256(path):
        raise CarverBlocked("S27 v2 TEST row 1374 active blocker snapshot hash drift")
...
def _status_payload(
    ...
    return {
        ...
        "standing_authorization": "S27_V2_STANDING_BOUNDED_MARKET_ORDER_TBBO_SPREAD_EVIDENCE_POLICY",
        "status": (
            "PASS_ROW1374_BOUNDED_DATABENTO_TBBO_SELECTED_NOT_RESULT"
            if not failed
            else "FAIL_CLOSED_ROW1374_BOUNDED_DATABENTO_TBBO_INCOMPLETE_NOT_RESULT"
        ),
        ...
        "provider_api_access": "ROW1374_ONLY_BOUNDED_DATABENTO_TBBO",
        "downloads": "ROW1374_ONLY_BOUNDED_TBBO_WINDOW",
        "broader_test_continuation": "NO",
...
def _provenance_text(status: dict[str, Any]) -> str:
    return f"""# S27 V2 2023 TEST Row 1374 Market-Order TBBO Evidence Provenance
...
Provider access was limited to the row-1374 ZNM3 TBBO quote window from the active fail-closed TEST artifact. This is not broad provider access, not a general historical download, not result evidence, and not source-faithful evidence.
"""


def _process_record(status: dict[str, Any]) -> str:
    return f"""# S27 V2 ZN 2023 TEST Row 1374 Market-Order TBBO Evidence
...
This record is written under `S27_V2_2023_TEST_ROW1374_BOUNDED_MARKET_ORDER_TBBO_EVIDENCE_AND_CONTINUATION_GATE` and the standing bounded market-order TBBO spread evidence policy.
