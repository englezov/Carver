...
RUN_ID = "20260612_S27_V2_2023_TEST_ROW2_ZNH3_MARKET_SPREAD_TBBO"
AUTHORIZATION = "S27_V2_BOUNDED_DATABENTO_ROW2_MARKET_SPREAD_EVIDENCE_ACQUISITION"
...
OUTPUT_ROOT = (
    ...
    / "ZN"
    / "20260612_2023_test_row2_znh3_tbbo"
)
PROCESS_RECORD = (
    ROOT
    / "docs"
    / "process"
    / "CARVER_S27_ZN_V2_2023_TEST_ROW2_MARKET_SPREAD_DATABENTO_TBBO_EVIDENCE_2026-06-12.md"
)
...
class Row2MarketSpreadTBBOAcquisitionConfig:
...
def main() -> None:
    config = Row2MarketSpreadTBBOAcquisitionConfig(
        ...
        market_order_side=MARKET_ORDER_SIDE,
    ...
    status = run_row2_tbbo_acquisition(config)
    print(status["status"])
    print(f"output_root={OUTPUT_ROOT.relative_to(ROOT).as_posix()}")
    print(f"selected_executable_market_fill_price={status.get('selected_executable_market_fill_price', 'NA')}")


def run_row2_tbbo_acquisition(config: Row2MarketSpreadTBBOAcquisitionConfig) -> dict[str, Any]:
    ...
    try:
        if raw_dbn.exists() or raw_csv.exists():
            raise CarverBlocked("Row-2 TBBO raw outputs already exist; refusing to make a second provider request")
...
def _validate_config(config: Row2MarketSpreadTBBOAcquisitionConfig) -> None:
    if not config.execution_authorized:
        raise CarverBlocked("S27 v2 row-2 market-spread TBBO acquisition is not authorized")
    if config.lane_class != LANE_CLASS:
        raise CarverBlocked("S27 v2 row-2 market-spread TBBO acquisition is source-native futures only")
    if config.provider != PROVIDER or config.dataset != DATASET or config.schema != SCHEMA:
        raise CarverBlocked("S27 v2 row-2 market-spread acquisition must use DataBento GLBX.MDP3 TBBO")
    if config.stype_in != STYPE_IN or config.raw_symbol != RAW_SYMBOL:
        raise CarverBlocked("S27 v2 row-2 market-spread acquisition is locked to raw symbol ZNH3")
    if config.market_order_side != MARKET_ORDER_SIDE:
        raise CarverBlocked("S27 v2 row-2 market-spread acquisition is locked to SELL market order evidence")
    if config.fill_timestamp_utc != FILL_TIMESTAMP_UTC:
        raise CarverBlocked("S27 v2 row-2 market-spread acquisition is locked to the audited fill timestamp")
    if config.request_start_utc != REQUEST_START_UTC or config.request_end_utc != REQUEST_END_UTC:
        raise CarverBlocked("S27 v2 row-2 market-spread acquisition window drift")
    if config.broader_test_continuation or config.result_or_pnl_emission:
        raise CarverBlocked("S27 v2 row-2 market-spread acquisition must not continue TEST or emit PnL/results")


def _request_manifest_payload(config: Row2MarketSpreadTBBOAcquisitionConfig) -> dict[str, Any]:
    return {
        ...
        "market_order_side": config.market_order_side,
        ...
        "request_count": 1,
        "scope": "BOUNDED_ROW2_MARKET_ORDER_SPREAD_EVIDENCE_ONLY",
...
def _select_latest_non_crossed_quote_at_or_before_fill(rows: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    for row in rows:
        ...
        if (
            ...
            row = dict(row)
            row["quote_age_seconds"] = age_seconds
        ...
        return [
            {
                **base,
                "selected_quote_ts_event": "",
                "quote_age_seconds": "",
                "provider_condition_status": "FAIL_CLOSED_NO_FRESH_NON_CROSSED_QUOTE_AT_OR_BEFORE_FILL",
    ...
    return [
        {
            **base,
            ...
            "provider_condition_status": "PASS_SELECTED_FRESH_TBBO_QUOTE_AT_OR_BEFORE_FILL",
            "provider_error_count": 0,
            "provider_error_summary": "",
        }
    ]
...
def _selected_spread_rows(selected_row: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    executable_price = float(selected_row["bid_px_00"])
    row = {
        "row_index": 2,
        "fill_timestamp_utc": FILL_TIMESTAMP_UTC,
        "raw_symbol": RAW_SYMBOL,
...
def _status_payload(
    ...
    status = (
        ...
        else "FAIL_CLOSED_BOUNDED_DATABENTO_TBBO_ROW2_SPREAD_EVIDENCE_NOT_SELECTED"
    )
...
def _provenance_text(status: dict[str, Any]) -> str:
    return f"""# S27_V2 2023 TEST Row-2 ZNH3 Market-Order Spread TBBO Provenance
...
This is one bounded DataBento Historical `tbbo` request for ZNH3 around the audited row-2 SELL market-fill candidate timestamp `2023-01-03T02:00:00Z`.
...
executable_market_fill_price_source = {status["executable_market_fill_price_source"]}
...
"""
        ...
        evidence = "No accepted fresh non-crossed TBBO quote was selected; row-2 SELL market spread remains fail-closed.\n"
    return f"""# CARVER S27 ZN V2 2023 TEST Row-2 Market Spread DataBento TBBO Evidence
