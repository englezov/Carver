...
PROCESS_DOC = ROOT / "docs/process/CARVER_S27_ZN_V2_PRE2023_MULTI_ROW_PACK_BUILD_RESULT_2026-06-11.md"

AUTHORIZATION = "S27_V2_PRE2023_LOCAL_ONLY_MULTI_ROW_DEVELOPMENT_RECON_PACK_AND_RUNNER_IMPLEMENTATION_GATE"
STATUS = "LOCAL_PRE2023_MULTI_ROW_DEV_RECON_PACK_DECLARED_NOT_RESULT"
VALUATION_CONVENTION_LABEL = "SOURCE_NATIVE_ENGINEERING_VALUATION_ASSUMPTION_NOT_BOOK_EXPLICIT"

DECISION_TIMESTAMPS = ("2022-01-03T01:00:00Z", "2022-01-03T03:00:00Z")
FILL_TIMESTAMPS = ("2022-01-03T02:00:00Z", "2022-01-03T04:00:00Z")
VALUATION_TIMESTAMPS = ("2022-01-03T03:00:00Z", "2022-01-03T05:00:00Z")
COPY_FAMILIES = (
...
def main() -> None:
    ...
    _write_json(OUTPUT_PACK / "S27_V2_PRE2023_MULTI_ROW_DECLARED_INPUT_PACK_MANIFEST.json", manifest)
    _write_sha256s(OUTPUT_PACK / "S27_V2_PRE2023_MULTI_ROW_DECLARED_INPUT_PACK_SHA256SUMS.txt", OUTPUT_PACK)
...
def _hourly_pack_row(row: dict[str, str], role: str, index: int) -> dict[str, Any]:
    return {
        ...
        "row_locator": f"S27V2_PRE2023_MULTI_ROW_ZN_HOURLY_{role}_{index:04d}_{row['derived_completed_bar_end_utc'].replace('-', '').replace(':', '')}_{row['raw_symbol']}",
...
def _manifest(hourly: dict[str, dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        "artifact": "S27_V2_PRE2023_MULTI_ROW_DECLARED_INPUT_PACK_MANIFEST",
        "authorization": AUTHORIZATION,
        "status": STATUS,
        "lane": "SOURCE_NATIVE_FUTURES",
        "selected_slice_rule": "MINIMUM_OLDEST_2022_MULTI_ROW_DEV_RECON_AFTER_WARMUPS_PRESERVE_2023_FOR_TEST",
...
def _process_doc(manifest: dict[str, Any]) -> str:
    return f"""# S27_V2 Pre-2023 Multi-Row Development/Reconciliation Pack Build Result
