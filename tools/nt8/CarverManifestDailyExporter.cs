// Manifest-driven NinjaTrader Desktop daily export helper for Carver.
...
// source-native daily Last rows listed in the Carver seed manifest and writes
// NinjaTrader-native semicolon text files to the Carver quarantine.
...
{
    public class CarverManifestDailyExporter : Indicator
    {
        private const string LockedManifestId = "CARVER_PARTS_1_3_DAILY_SEED_S09_ZN_CONTINUOUS_READINESS";
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "CarverManifestDailyExporter";
                Description = "Disarmed Carver manifest daily Last export helper for source-native futures data.";
            ...
            else if (State == State.DataLoaded)
            {
                ...
                if (!ExecutionArmed)
                {
                    Print("CARVER_MANIFEST_DAILY_EXPORT_PREPARED_NOT_ARMED rows=" + ManifestRows.Length.ToString(CultureInfo.InvariantCulture));
                    return;
                }
        ...
        private void ValidateConfiguration()
        {
            if (ExpectedManifestId != LockedManifestId)
                throw new InvalidOperationException("Carver manifest export blocked: manifest id drift.");
            ...
            if (!string.Equals(requestedRoot, lockedRoot, StringComparison.OrdinalIgnoreCase))
                throw new InvalidOperationException("Carver manifest export blocked: output root must stay in the native daily quarantine.");
        ...
        {
            if (row.Root != "ZN")
                throw new InvalidOperationException("Carver manifest export blocked: first helper is locked to the ZN chain.");
            if (row.NinjaTraderSymbol != "ZN " + MonthCode(row.ContractMonth))
                throw new InvalidOperationException("Carver manifest export blocked: NinjaTrader symbol drift.");
            if (!row.NativeFile.Equals(@"ZN\ZN " + row.ContractMonth + ".Last.txt", StringComparison.Ordinal))
                throw new InvalidOperationException("Carver manifest export blocked: native file drift.");
        ...
        private void RequestRow(ExportRow row)
        {
            ...
            request.Request(new Action<BarsRequest, ErrorCode, string>((bars, errorCode, errorMessage) =>
            {
                if (errorCode != ErrorCode.NoError)
                    throw new InvalidOperationException("Carver manifest export blocked: bars request failed for " + row.NinjaTraderSymbol + ": " + errorCode + " " + errorMessage);

                WriteNativeExport(row, bars);
                pendingRequests -= 1;
                Print("CARVER_MANIFEST_DAILY_EXPORT_WRITTEN " + row.NativeFile + " pending=" + pendingRequests.ToString(CultureInfo.InvariantCulture));
            }));
        }
        ...
        private void WriteNativeExport(ExportRow row, BarsRequest bars)
        {
            ...
            if (string.IsNullOrWhiteSpace(outputDirectory))
                throw new InvalidOperationException("Carver manifest export blocked: output directory missing.");
            ...
            if (lines.Count == 0)
                throw new InvalidOperationException("Carver manifest export blocked: no completed daily rows returned for " + row.NinjaTraderSymbol);
        ...
        private static DateTime ParseDate(string value)
        {
            ...
            if (!DateTime.TryParseExact(value, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out parsed))
                throw new InvalidOperationException("Carver manifest export blocked: date must use yyyy-MM-dd.");
            return parsed;
        }
        ...
        private static string MonthCode(string contractMonth)
        {
            ...
            if (contractMonth == "06-26")
                return "JUN26";
            throw new InvalidOperationException("Carver manifest export blocked: undeclared contract month.");
        }
    }
}
