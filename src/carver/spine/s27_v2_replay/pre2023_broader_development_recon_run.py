...
AUTHORIZATION = "S27_V2_CONSOLIDATED_LOCAL_ONLY_PRE2023_BROADER_DEVELOPMENT_RECON_IMPLEMENTATION_AND_RUN_GATE"
STATUS = "S27_V2_PRE2023_BROADER_DEVELOPMENT_RECON_RUN_EMITTED_NOT_RESULT"
...
PACK_RELATIVE_PATH = (
    ...
    "20260611_pre2023_broader_dev_recon_2022_first_session_declared_pack"
)
OUTPUT_RELATIVE_PATH = (
    "docs/researchops/s27_v2_local_replay_runs/ZN/"
    "20260611_pre2023_broader_dev_recon_2022_first_session_run"
)
_PACK_PATH = (_REPO_ROOT / PACK_RELATIVE_PATH).resolve()
_OUTPUT_PATH = (_REPO_ROOT / OUTPUT_RELATIVE_PATH).resolve()
_MANIFEST_NAME = "S27_V2_PRE2023_BROADER_DEV_RECON_DECLARED_INPUT_PACK_MANIFEST.json"
_EXPECTED_MANIFEST_SHA256 = "0937B1F499A67C488B0F02B7CD998D21D1DD74797BB02001D0E8AFBB581CEC86"
...
class Pre2023MultiRowDevelopmentReconRunBundle:
    ...
    def validate(self) -> None:
        ...
        if self.row_count != 4 or self.final_position_contracts != 15:
            raise CarverBlocked("S27 v2 pre-2023 multi-row run expected state is not locked")
        if not isclose(self.cumulative_gross_pnl_amount, -3359.375, rel_tol=0.0, abs_tol=1e-9):
            raise CarverBlocked("S27 v2 pre-2023 multi-row cumulative gross PnL is not locked")
        if not isclose(self.cumulative_commission_amount, 34.5, rel_tol=0.0, abs_tol=1e-9):
        ...
        if not isclose(self.cumulative_net_pnl_amount, -3393.875, rel_tol=0.0, abs_tol=1e-9):
...
def run_pre2023_broader_development_recon(
...
def _verify_manifest(manifest: dict[str, Any], rows: dict[str, list[dict[str, str]]], pack_path: Path) -> None:
    if manifest["status"] != "LOCAL_PRE2023_BROADER_DEV_RECON_PACK_DECLARED_NOT_RESULT":
    ...
    if len(manifest["decision_fill_mark_plan"]) != 4:
        raise CarverBlocked("S27 v2 pre-2023 broader pack must be the minimum four-row first-session slice")
    ...
    if len(rows["hourly_decision_completed_bar.csv"]) != 4 or len(rows["hourly_fill_completed_bar.csv"]) != 4:
        raise CarverBlocked("S27 v2 pre-2023 broader pack must contain four decision/fill rows")
    if len(rows["valuation_mark_completed_bar.csv"]) != 4:
        raise CarverBlocked("S27 v2 pre-2023 broader pack must contain four valuation marks")
...
def _write_artifacts(output_root: Path, pack_path: Path, manifest: dict[str, Any], computed: dict[str, list[dict[str, Any]]]) -> None:
    ...
    run_manifest = {
        "artifact": "S27_V2_PRE2023_BROADER_DEVELOPMENT_RECON_RUN_MANIFEST",
        "status": STATUS,
    ...
    evidence_manifest = {
        "artifact": "S27_V2_PRE2023_BROADER_DEVELOPMENT_RECON_EVIDENCE_MANIFEST",
        "status": "LOCAL_BROADER_DEV_RECON_EVIDENCE_MANIFEST_METADATA_NOT_RESULT",
    ...
    trusted_bundle = {
        "artifact": "S27_V2_PRE2023_BROADER_DEVELOPMENT_RECON_TRUSTED_BUNDLE_METADATA",
        "status": "LOCAL_BROADER_DEV_RECON_TRUSTED_BUNDLE_METADATA_NOT_RESULT_NOT_PROMOTION",
        "run_manifest_hash": _sha256(output_root / "run_manifest.json"),
