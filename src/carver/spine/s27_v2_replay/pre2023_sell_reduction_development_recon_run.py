...
AUTHORIZATION = "S27_V2_CONSOLIDATED_LOCAL_ONLY_PRE2023_SELL_REDUCTION_DEVELOPMENT_RECON_IMPLEMENTATION_AND_RUN_GATE"
STATUS = "S27_V2_PRE2023_SELL_REDUCTION_DEVELOPMENT_RECON_RUN_EMITTED_NOT_RESULT"
...
PACK_RELATIVE_PATH = (
    ...
    "20260611_pre2023_sell_reduction_dev_recon_2022_first_reduction_declared_pack"
)
OUTPUT_RELATIVE_PATH = (
    "docs/researchops/s27_v2_local_replay_runs/ZN/"
    "20260611_pre2023_sell_reduction_dev_recon_2022_first_reduction_run"
)
_PACK_PATH = (_REPO_ROOT / PACK_RELATIVE_PATH).resolve()
_OUTPUT_PATH = (_REPO_ROOT / OUTPUT_RELATIVE_PATH).resolve()
_MANIFEST_NAME = "S27_V2_PRE2023_SELL_REDUCTION_DEV_RECON_DECLARED_INPUT_PACK_MANIFEST.json"
_EXPECTED_MANIFEST_SHA256 = "611DF2F8A6AB08A2B2B62E39FDFC3FA883F0196A2A0FA9CE0572FAA1131045F6"
...
class Pre2023MultiRowDevelopmentReconRunBundle:
    ...
    def validate(self) -> None:
        ...
        if self.row_count != 44 or self.final_position_contracts != 34:
            raise CarverBlocked("S27 v2 pre-2023 multi-row run expected state is not locked")
        if not isclose(self.cumulative_gross_pnl_amount, -74515.625, rel_tol=0.0, abs_tol=1e-9):
            raise CarverBlocked("S27 v2 pre-2023 multi-row cumulative gross PnL is not locked")
        if not isclose(self.cumulative_commission_amount, 78.2, rel_tol=0.0, abs_tol=1e-9):
        ...
        if not isclose(self.cumulative_net_pnl_amount, -74593.825, rel_tol=0.0, abs_tol=1e-9):
...
def run_pre2023_sell_reduction_development_recon(
...
def _verify_manifest(manifest: dict[str, Any], rows: dict[str, list[dict[str, str]]], pack_path: Path) -> None:
    if manifest["status"] != "LOCAL_PRE2023_SELL_REDUCTION_DEV_RECON_PACK_DECLARED_NOT_RESULT":
    ...
    if len(manifest["decision_fill_mark_plan"]) != 44:
        raise CarverBlocked("S27 v2 pre-2023 sell-reduction pack must contain the 44-row first-reduction slice")
    ...
    if len(rows["hourly_decision_completed_bar.csv"]) != 44 or len(rows["hourly_fill_completed_bar.csv"]) != 44:
        raise CarverBlocked("S27 v2 pre-2023 sell-reduction pack must contain 44 decision/fill rows")
    if len(rows["valuation_mark_completed_bar.csv"]) != 44:
        raise CarverBlocked("S27 v2 pre-2023 sell-reduction pack must contain 44 valuation marks")
...
def _compute_rows(pack_path: Path, manifest: dict[str, Any], rows: dict[str, list[dict[str, str]]]) -> dict[str, list[dict[str, Any]]]:
    ...
    for index, (decision, fill, mark) in enumerate(zip(decisions, fills, marks, strict=True), 1):
        ...
        working_state_after = (
            ...
            if side == "NONE" or filled
            else "UNFILLED_LIMIT_ORDER_NOT_CARRIED_FAIL_CLOSED_WORKING_ORDER_LIFECYCLE"
        )
        out["transition"].append(_hash_row({"row_index": index, "starting_position_contracts": starting_position, "ending_position_contracts": current_position, "working_state_before": "EMPTY" if index == 1 else "NO_OPEN_WORKING_ORDER_CARRIED", "working_state_after": working_state_after, "same_session": decision["session_id"] == fill["session_id"] == mark["session_id"], "row_status": "LOCAL_WORKING_ORDER_TRANSITION_ROW_EMITTED_NOT_RESULT"}))
...
def _write_artifacts(output_root: Path, pack_path: Path, manifest: dict[str, Any], computed: dict[str, list[dict[str, Any]]]) -> None:
    ...
    run_manifest = {
        "artifact": "S27_V2_PRE2023_SELL_REDUCTION_DEVELOPMENT_RECON_RUN_MANIFEST",
        "status": STATUS,
    ...
    evidence_manifest = {
        "artifact": "S27_V2_PRE2023_SELL_REDUCTION_DEVELOPMENT_RECON_EVIDENCE_MANIFEST",
        "status": "LOCAL_SELL_REDUCTION_DEV_RECON_EVIDENCE_MANIFEST_METADATA_NOT_RESULT",
    ...
    trusted_bundle = {
        "artifact": "S27_V2_PRE2023_SELL_REDUCTION_DEVELOPMENT_RECON_TRUSTED_BUNDLE_METADATA",
        "status": "LOCAL_SELL_REDUCTION_DEV_RECON_TRUSTED_BUNDLE_METADATA_NOT_RESULT_NOT_PROMOTION",
        "run_manifest_hash": _sha256(output_root / "run_manifest.json"),
...
def _formula_limit(target_position: int, base_position: float, ewma5: float, sigma_price: float, multiplier_m: float, trend: float) -> float:
    if target_position <= 0 or trend <= 0.0:
        raise CarverBlocked("S27 v2 pre-2023 multi-row currently locks only positive trend-permitted targets")
...
def _round_limit(price: float, side: str) -> float:
    ...
    if side == "SELL":
        return ceil((price - 1e-12) / ZN_TICK_SIZE) * ZN_TICK_SIZE
    raise CarverBlocked("S27 v2 pre-2023 multi-row currently locks only buy/sell limit rounding")
