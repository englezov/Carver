...
AUTHORIZATION = "S27_V2_PRE2023_LOCAL_ONLY_MULTI_ROW_DEVELOPMENT_RECON_PACK_AND_RUNNER_IMPLEMENTATION_GATE"
STATUS = "S27_V2_PRE2023_MULTI_ROW_DEVELOPMENT_RECON_RUN_EMITTED_NOT_RESULT"
...
PACK_RELATIVE_PATH = (
    ...
    "20260611_pre2023_multi_row_dev_recon_2022_minimum_declared_pack"
)
OUTPUT_RELATIVE_PATH = (
    "docs/researchops/s27_v2_local_replay_runs/ZN/"
    "20260611_pre2023_multi_row_dev_recon_2022_minimum_run"
)
_PACK_PATH = (_REPO_ROOT / PACK_RELATIVE_PATH).resolve()
_OUTPUT_PATH = (_REPO_ROOT / OUTPUT_RELATIVE_PATH).resolve()
_MANIFEST_NAME = "S27_V2_PRE2023_MULTI_ROW_DECLARED_INPUT_PACK_MANIFEST.json"
_EXPECTED_MANIFEST_SHA256 = "362A9C7F9E06737169703383C868C11682A41C3310FAFC917097ADA46806268F"
...
class Pre2023MultiRowDevelopmentReconRunBundle:
    ...
    def validate(self) -> None:
        ...
        if self.row_count != 2 or self.final_position_contracts != 12:
            raise CarverBlocked("S27 v2 pre-2023 multi-row run expected state is not locked")
        if not isclose(self.cumulative_gross_pnl_amount, -1500.0, rel_tol=0.0, abs_tol=1e-9):
            raise CarverBlocked("S27 v2 pre-2023 multi-row cumulative gross PnL is not locked")
        if not isclose(self.cumulative_commission_amount, 27.6, rel_tol=0.0, abs_tol=1e-9):
        ...
        if not isclose(self.cumulative_net_pnl_amount, -1527.6, rel_tol=0.0, abs_tol=1e-9):
...
def run_pre2023_multi_row_development_recon(
...
def _verify_manifest(manifest: dict[str, Any], rows: dict[str, list[dict[str, str]]], pack_path: Path) -> None:
    if manifest["status"] != "LOCAL_PRE2023_MULTI_ROW_DEV_RECON_PACK_DECLARED_NOT_RESULT":
    ...
    if len(manifest["decision_fill_mark_plan"]) != 2:
        raise CarverBlocked("S27 v2 pre-2023 multi-row pack must be the minimum two-row slice")
    ...
    if len(rows["hourly_decision_completed_bar.csv"]) != 2 or len(rows["hourly_fill_completed_bar.csv"]) != 2:
        raise CarverBlocked("S27 v2 pre-2023 multi-row pack must contain two decision/fill rows")
    if len(rows["valuation_mark_completed_bar.csv"]) != 2:
        raise CarverBlocked("S27 v2 pre-2023 multi-row pack must contain two valuation marks")
...
def _compute_rows(pack_path: Path, manifest: dict[str, Any], rows: dict[str, list[dict[str, str]]]) -> dict[str, list[dict[str, Any]]]:
    ...
    out: dict[str, list[dict[str, Any]]] = {name: [] for name in ("runtime", "forecast", "position", "order", "transition", "fill", "cost", "pnl", "validation")}
...
def _write_artifacts(output_root: Path, pack_path: Path, manifest: dict[str, Any], computed: dict[str, list[dict[str, Any]]]) -> None:
    ...
    run_manifest = {
        "artifact": "S27_V2_PRE2023_MULTI_ROW_DEVELOPMENT_RECON_RUN_MANIFEST",
        "status": STATUS,
    ...
    evidence_manifest = {
        "artifact": "S27_V2_PRE2023_MULTI_ROW_DEVELOPMENT_RECON_EVIDENCE_MANIFEST",
        "status": "LOCAL_MULTI_ROW_EVIDENCE_MANIFEST_METADATA_NOT_RESULT",
    ...
    trusted_bundle = {
        "artifact": "S27_V2_PRE2023_MULTI_ROW_DEVELOPMENT_RECON_TRUSTED_BUNDLE_METADATA",
        "status": "LOCAL_MULTI_ROW_TRUSTED_BUNDLE_METADATA_NOT_RESULT_NOT_PROMOTION",
        "run_manifest_hash": _sha256(output_root / "run_manifest.json"),
