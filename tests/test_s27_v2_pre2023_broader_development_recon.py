...
from carver.spine.s27_v2_replay.pre2023_broader_development_recon_run import (
    NON_AUTHORIZATIONS,
    OUTPUT_RELATIVE_PATH,
    PACK_RELATIVE_PATH,
    _bundle_payload,
    run_pre2023_broader_development_recon,
)
...
def multi_row_bundle():
    return run_pre2023_broader_development_recon()
...
def test_multi_row_pack_declares_minimum_oldest_2022_slice():
    manifest = _json(PACK_ROOT, "S27_V2_PRE2023_BROADER_DEV_RECON_DECLARED_INPUT_PACK_MANIFEST.json")
    ...
    assert manifest["status"] == "LOCAL_PRE2023_BROADER_DEV_RECON_PACK_DECLARED_NOT_RESULT"
    assert manifest["selected_slice_rule"] == "MINIMUM_OLDEST_2022_BROADER_DEV_RECON_FIRST_SESSION_SEGMENT_AFTER_WARMUPS_PRESERVE_2023_FOR_TEST"
    assert [row["completed_timestamp_utc"] for row in decisions] == ["2022-01-03T01:00:00Z", "2022-01-03T03:00:00Z", "2022-01-03T05:00:00Z", "2022-01-03T07:00:00Z"]
    assert [row["completed_timestamp_utc"] for row in fills] == ["2022-01-03T02:00:00Z", "2022-01-03T04:00:00Z", "2022-01-03T06:00:00Z", "2022-01-03T08:00:00Z"]
    assert [row["completed_timestamp_utc"] for row in marks] == ["2022-01-03T03:00:00Z", "2022-01-03T05:00:00Z", "2022-01-03T07:00:00Z", "2022-01-03T09:00:00Z"]
...
def test_multi_row_run_carries_position_state_and_pnl(multi_row_bundle):
    ...
    assert multi_row_bundle.row_count == 4
    assert multi_row_bundle.final_position_contracts == 15
    ...
    assert [row["market_order_required"] for row in no_market] == ["FALSE", "FALSE", "FALSE", "FALSE"]
    assert [row["market_order_rows_emitted"] for row in no_market] == ["FALSE", "FALSE", "FALSE", "FALSE"]
    assert float(pnl[3]["cumulative_net_pnl_amount"]) == pytest.approx(-3393.875)
    assert multi_row_bundle.cumulative_net_pnl_amount == pytest.approx(-3393.875)
...
def test_multi_row_run_writes_hash_bound_metadata(multi_row_bundle):
    ...
    assert manifest["status"] == "S27_V2_PRE2023_BROADER_DEVELOPMENT_RECON_RUN_EMITTED_NOT_RESULT"
...
def test_multi_row_run_rejects_out_of_scope_paths(tmp_path):
    with pytest.raises(CarverBlocked, match="locked to the audited multi-row pack"):
        run_pre2023_broader_development_recon(tmp_path, RUN_ROOT)
    with pytest.raises(CarverBlocked, match="output root is locked"):
        run_pre2023_broader_development_recon(PACK_ROOT, tmp_path)
...
def test_multi_row_run_is_not_package_root_exported():
    assert "run_pre2023_broader_development_recon" not in getattr(package_root, "__all__", ())
    assert not hasattr(package_root, "run_pre2023_broader_development_recon")
