...
from carver.spine.s27_v2_replay.pre2023_extended_development_recon_run import (
    NON_AUTHORIZATIONS,
    OUTPUT_RELATIVE_PATH,
    PACK_RELATIVE_PATH,
    _bundle_payload,
    run_pre2023_extended_development_recon,
)
...
def multi_row_bundle():
    return run_pre2023_extended_development_recon()
...
def test_multi_row_pack_declares_minimum_oldest_2022_slice():
    manifest = _json(PACK_ROOT, "S27_V2_PRE2023_EXTENDED_DEV_RECON_DECLARED_INPUT_PACK_MANIFEST.json")
    ...
    assert manifest["status"] == "LOCAL_PRE2023_EXTENDED_DEV_RECON_PACK_DECLARED_NOT_RESULT"
    assert manifest["selected_slice_rule"] == "MINIMUM_OLDEST_2022_EXTENDED_DEV_RECON_TWO_SESSION_SEGMENT_AFTER_WARMUPS_PRESERVE_2023_FOR_TEST"
    assert [row["completed_timestamp_utc"] for row in decisions] == [
    ...
    assert [row["completed_timestamp_utc"] for row in fills] == [
    ...
    assert [row["completed_timestamp_utc"] for row in marks] == [
        "2022-01-03T03:00:00Z",
        "2022-01-03T05:00:00Z",
        "2022-01-03T07:00:00Z",
...
def test_multi_row_run_carries_position_state_and_pnl(multi_row_bundle):
    ...
    assert multi_row_bundle.row_count == 10
    assert multi_row_bundle.final_position_contracts == 33
    ...
    assert float(positions[2]["starting_position_contracts"]) == 12
    assert float(positions[2]["desired_position_contracts"]) == 14
    assert float(positions[2]["position_change_contracts"]) == 2
    assert float(orders[2]["limit_order_price"]) == pytest.approx(130.265625)
    assert fills[2]["fill_executed"] == "TRUE"
    assert float(fills[2]["fill_quantity"]) == 2
    assert float(fills[2]["position_after_fill"]) == 14
    assert float(positions[3]["starting_position_contracts"]) == 14
    assert float(positions[3]["desired_position_contracts"]) == 15
    assert float(positions[3]["position_change_contracts"]) == 1
    assert float(orders[3]["limit_order_price"]) == pytest.approx(130.25)
    assert fills[3]["fill_executed"] == "TRUE"
    assert float(fills[3]["fill_quantity"]) == 1
    assert float(fills[3]["position_after_fill"]) == 15
    assert float(positions[4]["starting_position_contracts"]) == 15
    assert float(positions[4]["desired_position_contracts"]) == 33
    assert float(positions[4]["position_change_contracts"]) == 18
    assert float(orders[4]["limit_order_price"]) == pytest.approx(130.234375)
    assert fills[4]["fill_executed"] == "TRUE"
    assert float(fills[4]["fill_quantity"]) == 18
    assert float(fills[4]["position_after_fill"]) == 33
    assert [float(row["position_change_contracts"]) for row in positions[5:]] == [0, 0, 0, 0, 0]
    assert [row["order_side"] for row in orders[5:]] == ["NONE", "NONE", "NONE", "NONE", "NONE"]
    assert [row["fill_executed"] for row in fills[5:]] == ["FALSE", "FALSE", "FALSE", "FALSE", "FALSE"]
    assert [row["market_order_required"] for row in no_market] == ["FALSE"] * 10
    assert [row["market_order_rows_emitted"] for row in no_market] == ["FALSE"] * 10
    assert float(pnl[-1]["cumulative_gross_pnl_amount"]) == pytest.approx(-30312.5)
    assert float(pnl[-1]["cumulative_commission_amount"]) == pytest.approx(75.9)
    assert float(pnl[-1]["cumulative_net_pnl_amount"]) == pytest.approx(-30388.4)
    assert multi_row_bundle.cumulative_net_pnl_amount == pytest.approx(-30388.4)
...
def test_multi_row_run_writes_hash_bound_metadata(multi_row_bundle):
    ...
    assert manifest["status"] == "S27_V2_PRE2023_EXTENDED_DEVELOPMENT_RECON_RUN_EMITTED_NOT_RESULT"
...
def test_multi_row_run_rejects_out_of_scope_paths(tmp_path):
    with pytest.raises(CarverBlocked, match="locked to the audited multi-row pack"):
        run_pre2023_extended_development_recon(tmp_path, RUN_ROOT)
    with pytest.raises(CarverBlocked, match="output root is locked"):
        run_pre2023_extended_development_recon(PACK_ROOT, tmp_path)
...
def test_multi_row_run_is_not_package_root_exported():
    assert "run_pre2023_extended_development_recon" not in getattr(package_root, "__all__", ())
    assert not hasattr(package_root, "run_pre2023_extended_development_recon")
