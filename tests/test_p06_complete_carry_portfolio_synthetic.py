...
from carver.spine.p06 import (  # noqa: E402
...
class P06CompleteCarryPortfolioSyntheticTests(unittest.TestCase):
    def setUp(self) -> None:
        self.as_of = datetime(2026, 5, 29, tzinfo=timezone.utc)
        self.completed_bar = CompletedBar(self.as_of)

    def test_p06_emits_complete_carry_desired_position_inputs_only(self) -> None:
        result = p06_complete_carry_portfolio_conformance(self.request())

        self.assertEqual(result.portfolio_id, P06_COMPLETE_CARRY_PORTFOLIO_ID)
        ...
        self.assertAlmostEqual(by_member["SYN_BOND_A"].final_capped_p06_forecast, 6.18)
        self.assertAlmostEqual(by_member["SYN_BOND_B"].final_capped_p06_forecast, -3.0)
        self.assertAlmostEqual(by_member["SYN_EQUITY_A"].final_capped_p06_forecast, FORECAST_CAP)
        self.assertAlmostEqual(by_member["SYN_ENERGY_A"].final_capped_p06_forecast, 1.53)
        for member in result.member_results:
            self.assertAlmostEqual(member.forecast_multiplier, member.final_capped_p06_forecast / P06_FORECAST_DIVISOR)
    ...
    def test_handcrafted_weights_follow_asset_group_member_taxonomy(self) -> None:
        weights = p06_handcrafted_instrument_weights(self.members())
    ...
    def test_p06_uses_strategy_ten_equal_weights_fdm_and_caps(self) -> None:
        result = p06_complete_carry_portfolio_conformance(self.request())
        equity = {member.member_id: member for member in result.member_results}["SYN_EQUITY_A"]

        self.assertEqual(equity.eligible_spans, (5, 20, 60, 120))
        self.assertAlmostEqual(equity.s10_forecast_block.rule_results[0].weight, 0.25)
        self.assertAlmostEqual(equity.s10_forecast_block.fdm, 1.04)
        self.assertAlmostEqual(equity.s10_forecast_block.pre_fdm_forecast, 20.0)
        self.assertGreater(equity.s10_forecast_block.post_fdm_forecast, FORECAST_CAP)
        self.assertEqual(equity.final_capped_p06_forecast, FORECAST_CAP)

    def test_p06_fails_closed_on_unresolved_locks_and_non_source_native_lane(self) -> None:
        ...
        with self.assertRaises(CarverBlocked):
            p06_complete_carry_portfolio_conformance(self.request(lane_class=LaneClass.CFD_ADAPTER))

    def test_p06_fails_closed_on_synthetic_input_and_timestamp_drift(self) -> None:
        forecasts = list(self.forecast_inputs())
        forecasts[0] = replace(forecasts[0], label="production_s10_forecast")
        with self.assertRaises(CarverBlocked):
            p06_complete_carry_portfolio_conformance(self.request(forecast_inputs=tuple(forecasts)))
    ...
    def test_p06_fails_closed_on_member_drop_or_carry_span_drift(self) -> None:
        with self.assertRaises(CarverBlocked):
            p06_complete_carry_portfolio_conformance(self.request(market_inputs=self.market_inputs()[:-1]))

        with self.assertRaises(CarverBlocked):
            p06_complete_carry_portfolio_conformance(self.request(eligible_carry_sets=self.eligible_carry_sets()[:-1]))

        bad_eligible = list(self.eligible_carry_sets())
        bad_eligible[0] = replace(bad_eligible[0], spans=(5, 60, 120))
        with self.assertRaises(CarverBlocked):
            p06_complete_carry_portfolio_conformance(self.request(eligible_carry_sets=tuple(bad_eligible)))

        forecasts = tuple(item for item in self.forecast_inputs() if not (item.member_id == "SYN_BOND_A" and item.span == 120))
        with self.assertRaises(CarverBlocked):
            p06_complete_carry_portfolio_conformance(self.request(forecast_inputs=forecasts))

    def test_p06_fails_closed_on_member_identity_taxonomy_and_prevalidation(self) -> None:
        members = list(self.members())
        members[0] = replace(members[0], member_id="SYN_BAD_CODE")
        with self.assertRaises(CarverBlocked):
            p06_complete_carry_portfolio_conformance(self.request(members=tuple(members)))
        ...
        with self.assertRaises(CarverBlocked):
            p06_complete_carry_portfolio_conformance(self.request(market_inputs=tuple(markets)))
    ...
    def test_p06_conformance_doc_records_boundary(self) -> None:
        text = (
            ROOT
            / "docs"
            / "process"
            / "CARVER_P06_COMPLETE_CARRY_PORTFOLIO_CONFORMANCE_2026-05-29.md"
        ).read_text(encoding="utf-8")

        self.assertIn("PROCESS_AND_SYNTHETIC_CODE_CARVER_P06_COMPLETE_CARRY_PORTFOLIO_CONFORMANCE_NOT_DATA_NOT_DIAGNOSTIC_NOT_BACKTEST", text)
        self.assertIn("Strategy Ten", text)
        self.assertIn("Appendix C", text)
        self.assertIn("desired position inputs only", text)
        self.assertIn("No real data", text)
        self.assertIn("no P07", text)

    def request(
        self,
        *,
        members: tuple[P06SyntheticMember, ...] | None = None,
        market_inputs: tuple[P06SyntheticMarketInput, ...] | None = None,
        forecast_inputs: tuple[P06SyntheticS10CarryForecastInput, ...] | None = None,
        eligible_carry_sets: tuple[P06EligibleCarrySet, ...] | None = None,
        source_locks: P06CompleteCarryPortfolioSourceLocks | None = None,
        lane_class: LaneClass = LaneClass.SOURCE_NATIVE_FUTURES,
    ) -> P06CompleteCarryPortfolioRequest:
        return P06CompleteCarryPortfolioRequest(
            ...
            eligible_carry_sets=eligible_carry_sets or self.eligible_carry_sets(),
            capital=TimedValue(50_000_000.0, self.as_of),
            target_risk=TimedValue(P06_BOOK_REFERENCE_TARGET_RISK, self.as_of),
            idm=TimedValue(P06_JUMBO_REFERENCE_IDM, self.as_of),
            source_locks=source_locks or self.locks(),
            rounding_policy=RoundingPolicy.TRUNCATE,
            lane_class=lane_class,
        )

    def members(self) -> tuple[P06SyntheticMember, ...]:
        return (
    ...
    def member(self, code: str, asset_class: str, group: str, multiplier: float) -> P06SyntheticMember:
        return P06SyntheticMember(
            member_id=code,
    ...
    def market_inputs(self) -> tuple[P06SyntheticMarketInput, ...]:
        return (
    ...
    def market(self, member_id: str, price: float, risk: float, daily_price_risk: float, cost: float) -> P06SyntheticMarketInput:
        return P06SyntheticMarketInput(
            member_id=member_id,
    ...
    def eligible_carry_sets(self) -> tuple[P06EligibleCarrySet, ...]:
        return (
            P06EligibleCarrySet("SYN_BOND_A", (20, 60, 120), SourceRuleStatus.LOCKED),
            P06EligibleCarrySet("SYN_BOND_B", (120,), SourceRuleStatus.LOCKED),
            P06EligibleCarrySet("SYN_EQUITY_A", (5, 20, 60, 120), SourceRuleStatus.LOCKED),
            P06EligibleCarrySet("SYN_ENERGY_A", (60, 120), SourceRuleStatus.LOCKED),
        )

    def forecast_inputs(self) -> tuple[P06SyntheticS10CarryForecastInput, ...]:
        values = {
            "SYN_BOND_A": {20: 5.0, 60: 6.0, 120: 7.0},
            "SYN_BOND_B": {120: -3.0},
            "SYN_EQUITY_A": {5: 20.0, 20: 20.0, 60: 20.0, 120: 20.0},
            "SYN_ENERGY_A": {60: 1.0, 120: 2.0},
        }
        return tuple(
            P06SyntheticS10CarryForecastInput(
                member_id=member_id,
                ...
                label="synthetic_s10_carry_forecast_block_output",
    ...
    def locks(self) -> P06CompleteCarryPortfolioSourceLocks:
        return P06CompleteCarryPortfolioSourceLocks(
            ...
            eligible_carry_span_set_status=SourceRuleStatus.LOCKED,
            forecast_weight_status=SourceRuleStatus.LOCKED,
            carry_fdm_status=SourceRuleStatus.LOCKED,
