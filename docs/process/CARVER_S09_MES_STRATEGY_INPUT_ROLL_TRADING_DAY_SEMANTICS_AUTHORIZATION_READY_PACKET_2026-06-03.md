# S09 MES Strategy Input roll_trading_day_semantics Authorization Ready Packet
...
- blocking_reason: Roll trading-day semantics are not locked to completed bars.
- next_action: Lock provider dates to completed trading dates before strategy computation.

operator authorization wording:

Operator authorizes only `roll_trading_day_semantics` source-native evidence locking for
