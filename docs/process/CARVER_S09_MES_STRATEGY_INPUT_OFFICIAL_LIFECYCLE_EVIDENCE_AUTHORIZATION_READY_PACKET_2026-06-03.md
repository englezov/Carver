# S09 MES Strategy Input official_lifecycle_evidence Authorization Ready Packet
...
- blocking_reason: Official per-contract lifecycle evidence is not locked for the machinery slice.
- next_action: Lock per-contract lifecycle evidence from explicitly authorized source-native sources.

operator authorization wording:

Operator authorizes only `official_lifecycle_evidence` source-native evidence locking for
