# S09 MES Strategy Input historical_mes_cost_values Authorization Ready Packet
...
- blocking_reason: Historical MES cost components are not locked from source-native evidence.
- next_action: Lock exchange, clearing/regulatory, broker, and spread/slippage cost evidence.

operator authorization wording:

Operator authorizes only `historical_mes_cost_values` source-native evidence locking for
