...
- `web_chart_api.py` can read a JSON chart response only from a Git-ignored quarantine directory and normalize it through the existing synthetic chart validator. The response envelope must echo the exact locked request payload and provider identity before any bars are admitted.
